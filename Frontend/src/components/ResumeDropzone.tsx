...
export const ResumeDropzone = ({ onFileUrlChange, className, playgroundView = false }: { onFileUrlChange: (fileUrl: string) => void; className?: string; playgroundView?: boolean }) => {
    ...
    >
      ...
      >
        ...
        <div className="pt-4">
          {!hasFile ? (
            <>
              <label className={`within-outline-theme-purple cursor-pointer rounded-full px-6 pb-2.5 pt-2 font-semibold shadow-sm, ${playgroundView ? "border" : "bg-primary"}`}>
                Browse file
            ...
            <>
              ...
              <p className={` text-gray-500,${!playgroundView && "mt-6"}`}>Note: {!playgroundView ? "Import" : "Parser"} works best on single column resume</p>
            </>
          )}
        </div>
      </div>
